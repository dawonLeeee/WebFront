...
const brand = "스타벅스";
    ...
    const product = {
        ...
        'information' : function(){
            ...
            // 해당 객체의 변수(key)가 아닌 전역변수가 우선순위를 가진다
    ...
    // -> 객체에 추가된다
    product.madeIn = "korea";
    div1.innerHTML += "product.madeIn : " + product.madeIn + "<br>";
    ...
    // 객체명['key'] 방법으로 객체가 가지고있는 모든 값 출력하기
...
document.getElementById("btn2").addEventListener("click", function(){
    ...
    // 생성자 함수 호출 : new 생성자함수(매개변수)
    const stdList = [];
    stdList.push(new Student(1, 2, "홍길동"));
    stdList.push(new Student(2, 5, "김길수"));
    stdList.push(new Student(3, 7, "김뫄뫄"));
    stdList.push(new Student(1, 6, "이갑을"));
...
function Student(grade, ban, name) {
    ...
    // 속성
    // 생성자 함수에서의 this 
    // == new 연산자를 통해 만들어지는 객체의 key(앞으로 만들어질 객체)
    this.grade = grade;
    this.ban = ban;
    this.name = name;
...
document.getElementById("btn3").addEventListener("click", function(){
    ...
    // ** JSON 변환 시 객체의 기능은 포함하지 않는다!!(함수 XX)
